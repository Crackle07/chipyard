...
#ifndef ICL_RMW_PAGES
#define ICL_RMW_PAGES 16U
#endif
...
static void fill_words(unsigned int page, unsigned int seq, unsigned int words)
{
  for (unsigned int i = 0; i < words; i++) {
    io_buf[i] =
        ((uint64_t) page << 32) ^ ((uint64_t) seq << 16) ^ (0x3d100000ULL + i);
  }
}
...
int main(void)
{
  ...
  const unsigned int required = ICL_RMW_PAGES * ICL_LINE_SECTORS;
  ...
  printf("icl_rmw_warm pages %u line_sectors %u subpage_sectors %u\n",
         ICL_RMW_PAGES,
         ICL_LINE_SECTORS,
         ICL_SUBPAGE_SECTORS);
  for (unsigned int page = 0; page < ICL_RMW_PAGES; page++) {
    const uint64_t cycles = write_request(page, ICL_LINE_SECTORS, page);
    printf("icl_rmw_warm_fill page %u cycles %lu\n",
           page,
           (unsigned long) cycles);
  }
  for (unsigned int page = 0; page < ICL_RMW_PAGES; page++) {
    const uint64_t cycles =
        write_request(page, ICL_SUBPAGE_SECTORS, ICL_RMW_PAGES + page);
    printf("icl_rmw_warm_subpage page %u cycles %lu\n",
           page,
           (unsigned long) cycles);
  }
  return 0;
