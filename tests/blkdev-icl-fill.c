...
#define ICL_WRITES (2U * ICL_CAPACITY_LINES)
...
static void fill_line(unsigned int page)
{
  for (unsigned int i = 0; i < WORDS_PER_LINE; i++) {
    write_buf[i] = ((uint64_t) page << 32) ^ (0x1c1f0000ULL + i);
  }
}
...
int main(void)
{
  ...
  const unsigned int required = ICL_WRITES * ICL_LINE_SECTORS;
  ...
  printf("icl_fill capacity_lines %u line_sectors %u writes %u\n",
         ICL_CAPACITY_LINES,
         ICL_LINE_SECTORS,
         ICL_WRITES);
  for (unsigned int page = 0; page < ICL_WRITES; page++) {
    const uint64_t cycles = write_page(page);
    printf("icl_fill_write page %u cycles %lu\n",
           page,
